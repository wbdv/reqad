...
# Overridable from the environment so the meta-installer can inject them.
SSH_KEY="${SSH_KEY:-}"
...
case "${EL}" in
    8|9) ;;
    *)  echo -e "\n ${RED}ERROR: Detected operating system: ${OS}. Only Enterprise Linux 8 and 9 (Rocky, AlmaLinux, RHEL, Oracle, CentOS Stream) are supported.${NC}\n" 1>&2
        exit 1 ;;
esac
...
if [ "$1" == "--skip-update" ]; then
	...
	# Union of the old el8 and el9 lists. iptables is the only real EL difference:
	# EL8 ships the legacy iptables package, EL9 replaced it with iptables-nft.
	PKGS="bash-completion bind-utils binutils bzip2 cloud-init cloud-utils-growpart"
	PKGS="${PKGS} cronie curl dialog dnf-utils e2fsprogs expect git htop iptables-libs"
	PKGS="${PKGS} mlocate net-tools pigz policycoreutils-python-utils pv rsyslog screen"
	PKGS="${PKGS} sqlite systemd-timesyncd sysstat tar tmpwatch unzip vim-enhanced vnstat wget"
	if [ "${EL}" -ge 9 ]; then
	...
	# Ask rpm per package rather than counting matches against a magic threshold -
	# the old `rpm -qa | grep -E ... | wc -l` -lt 30/34 test silently skipped the
	# whole install whenever the count drifted.
	MISSING=""
	for PKG in ${PKGS}; do
	    rpm -q --quiet "${PKG}" || MISSING="${MISSING} ${PKG}"
	done
	if [ -n "${MISSING}" ]; then
	    echo -n "Install the missing packages        ";
	    (dnf install -y ${MISSING}) >> ./install_reqad.log 2>&1
	    echo -e "[ ${GREEN}DONE${NC} ]"
	fi
fi
...
" >> /root/.bashrc
    ...
    # vim80 on EL8, vim82 on EL9, vim91 on EL10 - resolve it instead of guessing
    VIMSYNTAX=$(ls -d /usr/share/vim/vim*/syntax 2>/dev/null | tail -n 1)
    if [ -d "${VIMSYNTAX}" ]; then
        (wget -q https://www.vim.org/scripts/download_script.php?src_id=19394 -O ${VIMSYNTAX}/nginx.vim) >> ./install_reqad.log 2>&1
        (wget -q https://raw.githubusercontent.com/tmatilai/vim-monit/master/syntax/monitrc.vim -O ${VIMSYNTAX}/monit.vim) >> ./install_reqad.log 2>&1
    fi
    echo 'syntax on
