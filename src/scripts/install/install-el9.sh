...
if [[ "$(rpm -E %rhel 2>/dev/null)" != "9" ]]
    then echo -e "\n ${RED}ERROR: Detected operating system: ${OS}. Only Enterprise Linux 9 (Rocky, AlmaLinux, RHEL, Oracle, CentOS Stream) is supported.${NC}\n" 1>&2
    exit 1
fi
...
if [ "$1" == "--skip-update" ]; then
	...
	RPMNR=$(rpm -qa | grep -E "^dnf-utils|^git|^wget|^policycoreutils-python-utils|^pigz|^htop|^screen|^tar|^bind-utils|^unzip|^mlocate|^net-tools|^e2fsprogs|^bzip2|^curl|^cloud-init|^cloud-utils-growpart|^vim-enhanced|^systemd-timesyncd|^expect|^rsyslog|^pv|^dialog|^sqlite|^tmpwatch|^iptables-libs|^iptables-nft|^bash-completion|^sysstat|^vnstat" | wc -l)
	if [ "${RPMNR}" -lt 34 ]; then
	    echo -n "Install the missing packages        ";
	    (dnf install -y cronie dnf-utils git wget policycoreutils-python-utils pigz htop screen tar bind-utils unzip mlocate dnf-utils net-tools e2fsprogs bzip2 curl cloud-init cloud-utils-growpart vim-enhanced systemd-timesyncd expect rsyslog pv dialog sqlite tmpwatch iptables-libs iptables-nft sysstat vnstat) >> ./install_reqad.log 2>&1
	    echo -e "[ ${GREEN}DONE${NC} ]"
	fi
fi
...
if [ ! -f "/root/.vimrc" ]; then
    ...
    (wget -q https://www.vim.org/scripts/download_script.php?src_id=19394 -O /usr/share/vim/vim82/syntax/nginx.vim) >> ./install_reqad.log 2>&1
    (wget -q https://raw.githubusercontent.com/tmatilai/vim-monit/master/syntax/monitrc.vim -O /usr/share/vim/vim82/syntax/monit.vim) >> ./install_reqad.log 2>&1
    echo 'syntax on
...
(wget -q https://repo.reqad.net/csf.tgz) >> ./install_reqad.log
...
if ! systemctl is-active --quiet reqad; then
	...
	(dnf install -y https://repo.reqad.net/el9/RPMS/x86_64/reqad-repo-1.0.1-1.el9.noarch.rpm) >> ./install_reqad.log 2>&1
