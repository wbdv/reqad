...
# Install igbinary (php-pecl-igbinary) for every PHP version Reqad manages.
# Run manually as root after an RPM upgrade:   ./install-igbinary.sh
#
...
# the unprefixed Remi module package (php-pecl-igbinary); other versions use the
# Remi SCL prefix (e.g. php82-php-pecl-igbinary).
...
echo -ne "${YELLOW}"
echo -e "┌──────────────────────────────────────────────────────────────────────────────┐"
echo "│ Reqad install igbinary for all PHP versions     version ${VERSION} │"
echo -e "└──────────────────────────────────────────────────────────────────────────────┘\n"
echo -ne "${NC}"
...
# install_igbinary <label> <package> <fpm-service>
install_igbinary() {
    local label="$1" pkg="$2" service="$3"
    printf "igbinary for %-14s " "$label"
...
for v in $PHP_VERSIONS; do
    if [ "$v" == "$DEFAULT_PHP" ]; then
        install_igbinary "PHP $v (default)" "php-pecl-igbinary" "php-fpm"
    else
        suffix=$(echo "$v" | tr -d '.')
        install_igbinary "PHP $v" "php${suffix}-php-pecl-igbinary" "php${suffix}-php-fpm"
    fi
done
